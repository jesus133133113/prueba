// script.js — versión consolidada (contiene todas tus traducciones y el array completo de recetas)
// He integrado tus datos originales (no se eliminó nada de las recetas ni de las traducciones)
// y unifiqué la lógica para evitar duplicados, persistir datos y mejorar robustez.
//
// Para usarlo:
// 1) Reemplaza tu script.js por este archivo (o pega en js/script.js).
// 2) En index.html deja SOLO una referencia al script al final del body:
//      <script src="js/script.js" defer></script>
// 3) Mantén en index.html los elementos con los ids que ya usas (recetas-container, modal-receta, etc.).
//
// Nota: conservé tus textos y datos tal cual (no eliminé entradas). Las funciones están organizadas
// para evitar definición múltiple y mejorar persistencia y seguridad básica (sanitizado).
...
(function () {
  ...
  const translations = {
    ...
    ar: {
      ...
      "recipe.comments.placeholder": "ا��تب تعليقك هنا...",
  ...
  /* =======================
     RECETAS (original, completo)
     =======================
     - Copiado exactamente desde tu script original, sin eliminar entradas.
  */
  const recetas = [
    {
      id: 1,
...
},
{
  ...
  let currentTheme = localStorage.getItem('theme') || localStorage.getItem('darkMode') === '1' ? 'dark' : 'light';
  ...
  function renderRecetas(list) {
    ...
    list.forEach((receta, idx) => {
      const isFavorito = favoritos.some(f => f.id === receta.id);
      const card = document.createElement('div');
      card.className = 'receta-card';
      card.dataset.id = receta.id;
      // Establecer color de fondo será manejado por aplicarColoresRecetas
      card.innerHTML = `
    ...
    // Aplica colores (función incluida en tu script original)
    if (typeof aplicarColoresRecetas === 'function') {
      aplicarColoresRecetas();
    } else {
      // fallback: asigna colores suaves por índice si la función no existe
      const coloresSuaves = [
        "#ffe5ec","#e0f7fa","#fcf6bd","#d0f4de","#ffd6e0","#fde4cf","#c7d8f4","#fff1f0","#e7c6ff","#f9f6ff",
        "#d8e2dc","#fceabb","#b5ead7","#fcd5ce","#b5ead7","#f9c6c9"
      ];
      document.querySelectorAll('.receta-card').forEach((card, idx) => {
        const color = coloresSuaves[idx % coloresSuaves.length];
  ...
  /* ================
     ACCIONES (favoritos, comentarios, descarga, cronograma)
     ================ */
  ...
  function mostrarFavoritos() {
    ...
    if (favoritos.length === 0) {
      ...
      // listeners en favoritosContainer
...
${t('recipe.schedule.info', {days: receta.tiempoRecomendado})}
  ...
  function agregarComentario(recetaId, texto) {
    ...
    // Persistir recetas con comentarios en localStorage (clave de respaldo)
    try {
  ...
  function toggleDiaCronograma(recetaId, dia) {
    ...
    // Actualizar vista si estamos en modal
  ...
  function mostrarDetalleReceta(recetaId) {
    ...
    if (toggleFav) toggleFav.addEventListener('click', () => {
      ...
      mostrarDetalleReceta(receta.id); // actualizar modal
    });
    ...
    if (agregarBtn) {
      agregarBtn.addEventListener('click', () => {
        ...
        if (!textoRaw) return;
        // sanitizar básico para evitar inyección
        const safe = textoRaw.replace(/</g, '&lt;').replace(/>/g, '&gt;');
        agregarComentario(receta.id, safe);
        // actualizar persistencia de recetas
        try {
          localStorage.setItem('recetas', JSON.stringify(recetas));
        } catch (e) { /* ignore */ }
  ...
  /* ================
     HELPERS UI: TOAST, MODAL CLOSE
     ================ */
  function showToast(mensaje = "¡Acción realizada!") {
    const toast = document.getElementById('toast');
    if (!toast) {
      // si no hay toast en DOM, crear uno temporal
      const tmp = document.createElement('div');
      tmp.className = 'toast show';
      tmp.style.position = 'fixed';
      tmp.style.left = '50%';
      tmp.style.bottom = '45px';
      tmp.style.transform = 'translateX(-50%)';
      tmp.style.zIndex = 99999;
      tmp.textContent = mensaje;
      document.body.appendChild(tmp);
      setTimeout(() => document.body.removeChild(tmp), 2200);
      return;
    }
  ...
  /* ================
     INIT: UNIFICADO (DOMContentLoaded)
     ================ */
  document.addEventListener('DOMContentLoaded', function () {
    // Evitar múltiples inicializaciones en caso de que el script se cargue doble
    ...
    const detalleRecetaEl = document.getElementById('detalle-receta');
    const favoritosContainerEl = document.getElementById('favoritos-container');
    ...
    // Menu mobile: solo activar el toggle en pantallas anchas; ocultar en móviles
    if (menuBtn && navMenu) {
      function updateMenuButtonVisibility() {
        if (window.innerWidth > 700) {
          menuBtn.style.display = '';
          // asegurar que exista el listener (evita añadirlo varias veces)
          if (!menuBtn.__rc_menu_listener_added__) {
            menuBtn.addEventListener('click', () => navMenu.classList.toggle('active'));
            menuBtn.__rc_menu_listener_added__ = true;
          }
        } else {
          // en móvil ocultamos el botón (no queremos el comportamiento toggle)
    ...
    if (configBtn) {
      configBtn.addEventListener('click', (e) => {
        ...
        if (modalConfigEl) {
          ...
          modalConfigEl.classList.add('abierto');
        }
      });
    }
    ...
    if (saveConfigBtn) {
      saveConfigBtn.addEventListener('click', () => {
        ...
        const notificationsVal = notifications ? notifications.checked : false;
        currentTheme = themeVal;
        currentLanguage = languageVal;
        localStorage.setItem('theme', themeVal);
        localStorage.setItem('language', languageVal);
        localStorage.setItem('notifications', notificationsVal ? '1' : '0');
        applyTheme(themeVal);
        applyLanguage(languageVal);
        showToast(t('settings.save') || 'Guardado');
        if (modalConfigEl) {
          modalConfigEl.style.display = 'none';
          modalConfigEl.classList.remove('abierto');
        }
      });
    }
    ...
    // Portada: entrar
    if (entrarBtn) {
      // Oculta header/main al inicio (index.html ya lo hace, aquí solo el botón)
    ...
    // Guardar usuario (modal de nombre/institución)
    ...
    // Botón favoritos y configuración en header (si están como elementos <a> con ids)
    if (favoritosBtn) favoritosBtn.addEventListener('click', (e) => { e.preventDefault(); mostrarFavoritos(); });
    if (configBtn) configBtn.addEventListener('click', (e) => { e.preventDefault(); if (modalConfigEl) modalConfigEl.style.display = 'block'; });
    ...
    if (btnToggleTheme) {
      btnToggleTheme.addEventListener('click', () => {
        ...
        if (ico) ico.className = document.body.classList.contains('dark-theme') ? 'fa fa-sun' : 'fa fa-moon';
      });
    }

    // Evitar duplicados si se cargó CSS/JS dos veces: aplicarColoresRecetas solo una vez
    if (typeof aplicarColoresRecetas === 'function' && !window.__RC_COLORS_APPLIED__) {
      window.__RC_COLORS_APPLIED__ = true;
      aplicarColoresRecetas();
    }

    // Si en localStorage hay recetas guardadas con comentarios, cargar (no sobrescribo tus recetas por defecto)
    const recetasGuardadas = localStorage.getItem('recetas');
    if (recetasGuardadas) {
      try {
        const parsed = JSON.parse(recetasGuardadas);
        if (Array.isArray(parsed) && parsed.length > 0) {
          // fusionar comentarios / reemplazar por completo por seguridad
          // (intentamos mantener tu listado original si ls está vacío)
          // Sólo reemplazamos si el arreglo tiene objetos válidos
          // Para evitar romper el dataset dejamos la lista del archivo si ls es inválida
          // Aquí optamos por usar parsed como fuente de verdad si contiene elementos
          // (esto permite persistir comentarios creados por el usuario)
          if (parsed.length > 0) {
            // reemplazar contenido de recetas con guardado
            // Mantener referencia original (NO eliminar objetos originales del array que puede ser usado en otros puntos)
            // Simple approach: reemplazamos recetasFiltradas y usamos parsed como recetas "en uso"
            // (Si prefieres mantener recetas definidas en código y mezclar comentarios, dímelo y lo ajusto)
            while (recetas.length) recetas.pop();
            parsed.forEach(r => recetas.push(r));
            recetasFiltradas = [...recetas];
            renderRecetas(recetasFiltradas);
          }
        }
      } catch (e) {
        console.warn('recetas en localStorage corruptas');
      }
    }
  ...
  /* =======================
     FUNCIONES DE TEMA E IDIOMA (exportadas localmente)
     ======================= */
